...
library(dplyr)
library(tidyr)
path <- "~/Documents/github/forest_poverty_lite/data/"
lut.file <- "questions_LU_cat.csv"
csv.file <- "Data_Final_PROFOR_7_2_18.csv"
...
#Summarise the data
summary.byaid <- summarizer.list(data.outcome.subtype) %>% right_join(summary.byaid,by="aid")
...
save(data.biblio,data.interv,data.study,data.biomes,data.outcome,data.pathways, file = paste(path, "PROFOR_Evidence_Map_7_5_2018.RData", sep=""))
