...
map_data_final <- readRDS("data/map_data_final_7_5_18.rds")
load("data/PROFOR_Evidence_Map_7_5_2018.RData")
...
dir <- matrix(nrow=2,ncol=14)
rownames(dir) <- c("inc_con","cap_asset")
colnames(dir) <- int_type
out <- c("inc_con","cap_asset")

set <- select(data,aid,Int_type,Out_type_assigned,Outcome.direction) %>% distinct()
tot <- xtabs(~Int_type + Out_type_assigned + Outcome.direction,set)

direction <- read.csv("~/Documents/PROFOR/direction_counts.csv",header=TRUE)
direction$Out_type <- factor(direction$Out_type, levels=c("inc_con","cap_asset"))
direction$Direction <- factor(direction$Direction, levels=c("Negative","Neutral","Mixed","Positive","Unknown"))
direction$Int_type <- factor(direction$Int_type,levels=c("for_mgmt","agrofor","hab_mgmt","gov","emp","prod_cap","hum_cap","soc_cap","liv_alt","market","mark_acc","eco_ser","instit","non_mon"))
library(ggplot2)
pdf(file="Direction_counts.pdf",height=8,width=11)
ggplot(direction, aes(fill=Direction, y=Count, x=Out_type)) + geom_bar(stat='identity') + facet_grid(. ~ Int_type) + theme(axis.text.x = element_text(angle=45,vjust=0.5)) + scale_fill_brewer()
dev.off()
